package com.receta_medica.ejb.facade;
...
public abstract class Crud {
     ...
     * (Nota: la objeto entidad que retorn se encuentra ligada la base de datos y los cambios realizados este objeto se veran reflejados directamente en esta.)
     ...
     * @throws Exception si el objeto no esta en la base de datos o esta mal definido en paarmetro.
     */
    ...
    /**
     * Encuentra el elemento en la base de datos a partir de la PrimaryKey enviada en la <code>entity</code>.
     * @param <T> tipo de la entidad.
     * @param entity Entidad que se quiere buscar.
     * @return El registro de la base de datos, si se encuentra.
     * @throws Exception si el parametro entregado es <code>null</code> o si la Pk esta parcial o completamente <code>null</code>.
     */
    ...
    /**
     * Encuentra todos los registros de la tabla asociada que cumplan con los valor enviados en la clase entidad.<br/>
     * Si no se le asignan valores, es decir <code>null</code>, a la clase entidad se retorna la lista todos los registros de la tabla asociada.
     ...
     * @throws Exception si el parametro entregado es <code>null</code>, o si el paramtro no es una Clase entidad.
     */
...
//    }
    ...
    /**
     * Metodo para la autogeneracion del consecutivo de la Primary Key a una Clase entidad.<br/><br/>
     * Para que se pueda hacer la generacion del consecutivo se debe tener en cuenta las siguientes condiciones para la intancia de la Clase entidad:
     * <ul>
     *  - <il>Si la Clase entidad es <code>null</code> se levantra una exception de <code>NullPointerException</code>
     * </ul>
     * <ul>
     *  - <il>Si la Clase entidad no tiene llave compuesta, es decir tiene un unico campo con anotacion <code>@Id</code>, el campo Id debe ser <code>null</code>, 
     *    de lo contrario el valor que tenga sera reemplazado por el siguiente valor en la tabla.</il></br>
     * </ul>
     * <ul>
     *  - <il>Si la Clase entidad tiene llave compuesta y su valor es <code>null</code>, se levantara una exception de <code>IllegalArgumentException</code>.</il></br>
     * </ul>
     * <ul>
     *  - <il>
     *      Si el valor de la llave compuesta no es <code>null</code>: Solamente <b>debe</b> haber una llave primaria con valor <code>null</code> del conjunto de llaves primarias, 
     *      y esta sera a la cual se leasginara el valor generado. Si llega mas de una Llave primaria con valor <code>null</code> se levantara una exception de <code>IllegalArgumentException</code>.
     *    </il>
     * </ul>
     * @param <T> tipo de la entidad.
     * @param entity la entidad que se envia.
     * @throws Exception si el paramatero es <code>null</code>, si la instancia esta mal definida o si el parametro envidado no es de clase entidad.
     */
